...
```
    @if (session('success'))
        <div id="cart-notification" class="alert alert-success" style="position: fixed; top: 120px; right: 313px; background-color: #d4edda; color: #155724; border: 1px solid #c3e6cb; border-radius: 8px; padding: 10px 20px 10px 15px; z-index: 9999; box-shadow: 0 2px 8px rgba(0,0,0,0.1); opacity: 1; display: flex; align-items: center; justify-content: space-between; min-width: 250px;">
            <span>{{ session('success') }}</span>
            <button id="close-notification" style="background: transparent; border: none; color: #155724; font-weight: bold; font-size: 20px; line-height: 1; cursor: pointer; padding: 0 5px; margin-left: 15px;" aria-label="Close notification">&times;</button>
        </div>
    @endif

    {{-- Floating toast for unselected checkout --}}
    <div id="select-item-toast" class="alert alert-danger" style="position: fixed; top: 120px; right: 253px; background-color: #f8d7da; color: #721c24; border: 1px solid #f5c6cb; border-radius: 8px; padding: 10px 20px 10px 15px; z-index: 9999; box-shadow: 0 2px 8px rgba(0,0,0,0.1); opacity: 0; display: none; transition: opacity 0.5s ease; min-width: 250px; display: flex; align-items: center; justify-content: space-between;">
        <span>Please select at least one item to checkout.</span>
        <button id="close-toast" style="background: transparent; border: none; color: #721c24; font-weight: bold; font-size: 20px; line-height: 1; cursor: pointer; padding: 0 5px; margin-left: 15px;" aria-label="Close toast">&times;</button>
    </div>
            ...
            <tbody>
                ...
                @foreach ($cartItems as $item)
                    ...
                    <tr>
                        ...
                        <td class="align-middle text-center price">Rp {{ number_format($item->price, 0, ',', '.') }}</td>
                        <td class="align-middle text-center">
                            {{-- Form update quantity --}}
                            <form action="{{ route('cart.update', $item->cart_id) }}" method="POST" style="display:inline-block;">
                                @csrf
                                @method('PUT')
                                <input type="number" name="quantity" min="1" value="{{ $item->quantity }}"
                                    class="form-control form-control-sm" style="max-width: 70px; display:inline-block;">
                                <button type="submit" class="btn btn-sm btn-primary ms-1" title="Update Quantity">
                                    <i class="fas fa-sync-alt"></i>
                                </button>
                            </form>
                        </td>
                        <td class="align-middle text-center fw-semibold subtotal">Rp {{ number_format($subtotal, 0, ',', '.') }}</td>
                        <td class="align-middle text-center">
                            {{-- Form hapus item --}}
                            <form action="{{ route('cart.remove', $item->cart_id) }}" method="POST"
                                onsubmit="return confirm('Are you sure you want to remove this item?');" style="display:inline-block;">
                                @csrf
                                @method('DELETE')
                ...
                <tr>
                    <td colspan="6">
                        <div class="d-flex justify-content-end pe-3">
                            ...
                            <span class="fw-bold" id="total-display">Rp {{ number_format($total, 0, ',', '.') }}</span>
                        </div>
                    </td>
                </tr>
            </tfoot>
